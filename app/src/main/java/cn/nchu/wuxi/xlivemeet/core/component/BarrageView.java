package cn.nchu.wuxi.xlivemeet.core.component;
...
public class BarrageView extends View implements Runnable {
    ...
    public void addTextitem(String content) {//此方法可根据需求自行改动
        float x = getWidth() ;//随机x
        //float y = Math.abs(random.nextFloat() * (getHeight() - 50)) + 40;//稍微处理了一下y坐标，让字幕不紧贴边缘
        float y = Math.abs(random.nextFloat() * (getHeight() - 80)) + 40;//稍微处理了一下y坐标，让字幕不紧贴边缘
       // float y = getWidth();//稍微处理了一下y坐标，让字幕不紧贴边缘
...
//        int b = random.nextInt(255);
        Textitem textitem = new Textitem(content, x, y, step, Color.rgb(255, 255, 255));
        items.add(textitem);
