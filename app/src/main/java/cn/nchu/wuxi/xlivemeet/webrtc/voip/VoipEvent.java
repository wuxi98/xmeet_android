...
public class VoipEvent implements ISkyEvent {
    ...
    public void shouldStartRing(boolean isComing) {
        if (isComing) {
            Uri uri = Uri.parse("android.resource://" + XMeetApp.getInstance().getPackageName() + "/" + R.raw.incoming_call_ring);
            ringPlayer.play(XMeetApp.getInstance(), uri, true, AudioManager.STREAM_RING);
        } else {
            Uri uri = Uri.parse("android.resource://" + XMeetApp.getInstance().getPackageName() + "/" + R.raw.wr_ringback);
            ringPlayer.play(XMeetApp.getInstance(), uri, true, AudioManager.STREAM_RING);
        }
    }
